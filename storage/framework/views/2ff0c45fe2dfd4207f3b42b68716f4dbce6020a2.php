...
$(function() {
    ...
    var oTableBLN = $('#list-table-bln').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '<?php echo e(url('/penggajian'.(isset($_SERVER['QUERY_STRING']) ? '?'.$_SERVER['QUERY_STRING'] : ''))); ?>',
            method: 'POST',
            data: {
                '_token': '<?php echo e(csrf_token()); ?>',
                '_type': 'bln',
    ...
    $(document).on('submit', '.form-gaji', function(){
       ...
       $.ajax({
    		...
    		url			: frm.attr('action'),
            data        : frm.serialize(),
    		beforeSend	: function(xhr) { frm.find('.overlay').show(); frm.find('.btn-save').attr('disabled', true);frm.find('.btn-save').text('<?php echo e(trans('global.saving')); ?>'); },
    		success		: function(dt){
    	...
    	}).done(function(){ frm.find('.overlay').hide(); frm.find('.btn-save').attr('disabled', false);frm.find('.btn-save').text('<?php echo e(trans('global.save')); ?>'); frm.find('.btn-close').click(); loading(0);});
        return false; 
    });
    ...
    $(document).on('click', '.btn-reset', function(){
        ...
        $.ajax({
    		...
    		success		: function(dt){
              if ( dt.status )
              {
                  oTableBRG.ajax.reload();  
                  oTableBLN.ajax.reload();
                  $('.modalGaji').modal('hide');
              }
              if ( dt.message ) initNotif(atob(dt.message));
        	},
   ...
   $(document).on('click', '#terapkan', function(){
    ...
    function initCalc()
    {
        ...
        var total = lembur = potongan = 0;
        $(document).on('input', '.form-bulanan .slip-input', function(){
            var tb = $(this).closest('.tb-slip');
            total = parseInt(tb.find('.total-gapok').data('gapok'));
            lembur = 0;
            tb.find('.input-jam-lembur').each(function(){
               if ($(this).val()) 
               {
                lbr = parseFloat(numeral($(this).val()).value()) * $(this).closest('tr').find('.input-value-lembur').val();
                if (lbr) $(tb).find($(this).data('target')).text('Rp ' + initNumber(lbr));
                lembur += lbr;
               } 
            });
            tb.find('.subtotal-1').text('Rp ' + initNumber(total+lembur));
            
            potongan = 0;            
            tb.find('.input-min').each(function(){
               if ($(this).val()) 
               {
                potongan+= parseFloat(numeral($(this).val()).value());
               }
            });
            tb.find('.total-potongan').text('Rp ' + initNumber(potongan));
            tb.find('.subtotal-2').text('Rp ' + initNumber(total+lembur-potongan));
            tb.find('[name=total]').val((total+lembur-potongan));
        });
    }
