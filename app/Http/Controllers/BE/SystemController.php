...
class SystemController extends BaseController
{
    ...
    */
    function saveMenu()
    {
        ...
        $status = $this->_saveData( new Menu(), [   
            //VALIDATOR
            "name" => "required|unique:menu". ($input['id'] ? ",name,".$input['id'] : '')
        ], $input);
                    ...
                    foreach($_POST['modules'] as $m)
                    {
                        if ( $rowJson = $this->updateModule($m, ['active'=>($_POST['act']=="true"?1:0)]) )
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    //delete
                                    Access::where("menu_id", $menu->id)->delete();
    ...
    */
    function level()
    {
        ...
        {
            return Datatables::of(Level::query())->addColumn('action', function ($r) { return $this->_buildAction($r->id, $r->name); })->make(true);
        }
    }
    ...
    */
    ...
    {
        return Response()->json([ 
            'status' => $this->_deleteData(new Level(), $id), 
            'message'=> $this->_buildNotification(true)
        ]);
    }
    ...
    function user()
    {
        ...
        else
        {
            return Datatables::of(User::query())
                             ->addColumn('action', function ($r) { return $this->_buildAction($r->id, $r->username); })
    ...
    */
    ...
    {
        return Response()->json([ 
            'status' => $this->_deleteData(new User(), $id), 
            'message'=> $this->_buildNotification(true)
        ]);
    }
