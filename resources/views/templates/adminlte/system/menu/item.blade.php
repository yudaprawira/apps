<li class="dd-item dd3-item" data-id="{{ $id }}" id="listnested-{{ $id }}">
    ...
    <div class="dd3-content clearfix">{{ $name }} 
        <div class="pull-right" style="margin-top: -2px;">
            ...
            <a href="#" class="btn btn-xs btn-primary btn-flat btn-add" title="{{ trans('global.add') }}" data-id="{{$id}}" data-name="{{ $name }}"><i class="glyphicon glyphicon-plus"></i></a>
            <a href="{{ BeUrl('system-menu/edit/'.$id) }}" title="{{ trans('global.edit') }}" class="btn btn-xs btn-success btn-flat btn-edit"><i class="glyphicon glyphicon-pencil"></i></a>
        ...
        <div class="modal fade modalConfirmation" id="modalConfirmation-{{$id}}" tabindex="-1" role="dialog" aria-labelledby="modalConfirmation-{{$id}}-Label" aria-hidden="true">
          <div class="modal-dialog">
            <div class="modal-content box">
              ...
              <div class="modal-footer">
                <button type="button" class="btn btn-default btn-flat" data-dismiss="modal">{{ trans('global.act_close') }}</button>
                <a href="{{ BeUrl('system-menu/delete/'.$id) }}" data-id="#listnested-{{ $id }}" class="btn btn-primary btn-flat btn-delete">{{ trans('global.delete') }}</a>
              </div>
